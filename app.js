const res=[
  {
    "image": "eighth",
    "name": "Pasta Paradise",
    "rating": "3.9",
    "food_type": "Indian",
    "Price_for_two": 1784,
    "location": "Hauz Khas Village",
    "Distance_from_Customer_house": "9.2",
    "alcohol": false,
    "Restaurant_open_time": 23,
    "Restaurant_close_time": 11,
    "offers": 50
  },
  {
    "image": "ninth",
    "name": "Farm to Table",
    "rating": "1.0",
    "food_type": "Italian",
    "Price_for_two": 1338,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "9.4",
    "alcohol": false,
    "Restaurant_open_time": 1,
    "Restaurant_close_time": 13,
    "offers": 10
  },
  {
    "image": "fourth",
    "name": "Mediterranean Magic",
    "rating": "4.0",
    "food_type": "Italian",
    "Price_for_two": 1567,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "6.2",
    "alcohol": false,
    "Restaurant_open_time": 22,
    "Restaurant_close_time": 10,
    "offers": 0
  },
  {
    "image": "fourth",
    "name": "Farm to Table",
    "rating": "2.7",
    "food_type": "Italian",
    "Price_for_two": 2127,
    "location": "Rajouri Garden",
    "Distance_from_Customer_house": "8.1",
    "alcohol": true,
    "Restaurant_open_time": 21,
    "Restaurant_close_time": 9,
    "offers": 60
  },
  {
    "image": "fourth",
    "name": "Farm to Table",
    "rating": "2.1",
    "food_type": "Thai",
    "Price_for_two": 1862,
    "location": "Lajpat Nagar",
    "Distance_from_Customer_house": "10.6",
    "alcohol": false,
    "Restaurant_open_time": 9,
    "Restaurant_close_time": 21,
    "offers": 60
  },
  {
    "image": "eighth",
    "name": "Golden Wok",
    "rating": "1.6",
    "food_type": "Indian",
    "Price_for_two": 621,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "10.2",
    "alcohol": false,
    "Restaurant_open_time": 18,
    "Restaurant_close_time": 6,
    "offers": 70
  },
  {
    "image": "sixth",
    "name": "Urban Bites",
    "rating": "2.2",
    "food_type": "Korean",
    "Price_for_two": 411,
    "location": "Karol Bagh",
    "Distance_from_Customer_house": "7.0",
    "alcohol": false,
    "Restaurant_open_time": 0,
    "Restaurant_close_time": 12,
    "offers": 0
  },
  {
    "image": "ninth",
    "name": "The Rustic Table",
    "rating": "4.8",
    "food_type": "Mexican",
    "Price_for_two": 266,
    "location": "Saket",
    "Distance_from_Customer_house": "10.7",
    "alcohol": false,
    "Restaurant_open_time": 22,
    "Restaurant_close_time": 10,
    "offers": 40
  },
  {
    "image": "seventh",
    "name": "Ocean Delights",
    "rating": "1.9",
    "food_type": "Korean",
    "Price_for_two": 1344,
    "location": "Lajpat Nagar",
    "Distance_from_Customer_house": "10.0",
    "alcohol": true,
    "Restaurant_open_time": 3,
    "Restaurant_close_time": 15,
    "offers": 0
  },
  {
    "image": "fourth",
    "name": "Savory Street",
    "rating": "3.1",
    "food_type": "Italian",
    "Price_for_two": 580,
    "location": "Connaught Place",
    "Distance_from_Customer_house": "3.3",
    "alcohol": true,
    "Restaurant_open_time": 1,
    "Restaurant_close_time": 13,
    "offers": 30
  },
  {
    "image": "third",
    "name": "Spice Symphony",
    "rating": "2.5",
    "food_type": "Italian",
    "Price_for_two": 465,
    "location": "Vasant Kunj",
    "Distance_from_Customer_house": "5.8",
    "alcohol": false,
    "Restaurant_open_time": 11,
    "Restaurant_close_time": 23,
    "offers": 40
  },
  {
    "image": "third",
    "name": "The Rustic Table",
    "rating": "2.0",
    "food_type": "Mexican",
    "Price_for_two": 502,
    "location": "Karol Bagh",
    "Distance_from_Customer_house": "4.9",
    "alcohol": false,
    "Restaurant_open_time": 18,
    "Restaurant_close_time": 6,
    "offers": 50
  },
  {
    "image": "fifth",
    "name": "Golden Wok",
    "rating": "3.9",
    "food_type": "French",
    "Price_for_two": 906,
    "location": "Saket",
    "Distance_from_Customer_house": "10.8",
    "alcohol": true,
    "Restaurant_open_time": 8,
    "Restaurant_close_time": 20,
    "offers": 40
  },
  {
    "image": "fifth",
    "name": "Sushi Spot",
    "rating": "5.0",
    "food_type": "Italian",
    "Price_for_two": 1999,
    "location": "Connaught Place",
    "Distance_from_Customer_house": "8.9",
    "alcohol": true,
    "Restaurant_open_time": 16,
    "Restaurant_close_time": 4,
    "offers": 0
  },
  {
    "image": "first",
    "name": "Farm to Table",
    "rating": "2.9",
    "food_type": "Thai",
    "Price_for_two": 2005,
    "location": "Vasant Kunj",
    "Distance_from_Customer_house": "7.2",
    "alcohol": false,
    "Restaurant_open_time": 5,
    "Restaurant_close_time": 17,
    "offers": 30
  },
  {
    "image": "ninth",
    "name": "Fusion Fiesta",
    "rating": "4.5",
    "food_type": "French",
    "Price_for_two": 154,
    "location": "Connaught Place",
    "Distance_from_Customer_house": "9.2",
    "alcohol": false,
    "Restaurant_open_time": 23,
    "Restaurant_close_time": 11,
    "offers": 0
  },
  {
    "image": "fifth",
    "name": "Ocean Delights",
    "rating": "3.5",
    "food_type": "Korean",
    "Price_for_two": 2310,
    "location": "Rajouri Garden",
    "Distance_from_Customer_house": "4.9",
    "alcohol": true,
    "Restaurant_open_time": 18,
    "Restaurant_close_time": 6,
    "offers": 40
  },
  {
    "image": "tenth",
    "name": "Breakfast Club",
    "rating": "1.5",
    "food_type": "Japanese",
    "Price_for_two": 831,
    "location": "Karol Bagh",
    "Distance_from_Customer_house": "1.9",
    "alcohol": false,
    "Restaurant_open_time": 8,
    "Restaurant_close_time": 20,
    "offers": 0
  },
  {
    "image": "tenth",
    "name": "Urban Bites",
    "rating": "1.5",
    "food_type": "French",
    "Price_for_two": 652,
    "location": "Dwarka",
    "Distance_from_Customer_house": "3.3",
    "alcohol": true,
    "Restaurant_open_time": 22,
    "Restaurant_close_time": 10,
    "offers": 60
  },
  {
    "image": "fifth",
    "name": "The Cozy Corner",
    "rating": "4.9",
    "food_type": "Korean",
    "Price_for_two": 2419,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "4.2",
    "alcohol": false,
    "Restaurant_open_time": 22,
    "Restaurant_close_time": 10,
    "offers": 30
  },
  {
    "image": "third",
    "name": "Mediterranean Magic",
    "rating": "4.1",
    "food_type": "Mediterranean",
    "Price_for_two": 1432,
    "location": "Vasant Kunj",
    "Distance_from_Customer_house": "5.9",
    "alcohol": false,
    "Restaurant_open_time": 17,
    "Restaurant_close_time": 5,
    "offers": 40
  },
  {
    "image": "fourth",
    "name": "Golden Wok",
    "rating": "3.3",
    "food_type": "Japanese",
    "Price_for_two": 625,
    "location": "Lajpat Nagar",
    "Distance_from_Customer_house": "5.2",
    "alcohol": true,
    "Restaurant_open_time": 1,
    "Restaurant_close_time": 13,
    "offers": 50
  },
  {
    "image": "fifth",
    "name": "Fusion Fiesta",
    "rating": "2.9",
    "food_type": "Japanese",
    "Price_for_two": 1444,
    "location": "Hauz Khas Village",
    "Distance_from_Customer_house": "10.6",
    "alcohol": false,
    "Restaurant_open_time": 8,
    "Restaurant_close_time": 20,
    "offers": 70
  },
  {
    "image": "third",
    "name": "Fusion Fiesta",
    "rating": "1.9",
    "food_type": "American",
    "Price_for_two": 357,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "3.3",
    "alcohol": true,
    "Restaurant_open_time": 18,
    "Restaurant_close_time": 6,
    "offers": 50
  },
  {
    "image": "eighth",
    "name": "The Green Plate",
    "rating": "2.7",
    "food_type": "French",
    "Price_for_two": 442,
    "location": "Saket",
    "Distance_from_Customer_house": "3.5",
    "alcohol": false,
    "Restaurant_open_time": 16,
    "Restaurant_close_time": 4,
    "offers": 90
  },
  {
    "image": "tenth",
    "name": "Burger Bliss",
    "rating": "2.2",
    "food_type": "Chinese",
    "Price_for_two": 2007,
    "location": "Saket",
    "Distance_from_Customer_house": "6.4",
    "alcohol": false,
    "Restaurant_open_time": 17,
    "Restaurant_close_time": 5,
    "offers": 40
  },
  {
    "image": "fourth",
    "name": "Street Eats",
    "rating": "3.2",
    "food_type": "French",
    "Price_for_two": 492,
    "location": "Rajouri Garden",
    "Distance_from_Customer_house": "1.3",
    "alcohol": true,
    "Restaurant_open_time": 12,
    "Restaurant_close_time": 0,
    "offers": 70
  },
  {
    "image": "fourth",
    "name": "Urban Bites",
    "rating": "2.3",
    "food_type": "French",
    "Price_for_two": 1217,
    "location": "Dwarka",
    "Distance_from_Customer_house": "7.5",
    "alcohol": false,
    "Restaurant_open_time": 10,
    "Restaurant_close_time": 22,
    "offers": 20
  },
  {
    "image": "fourth",
    "name": "The Cozy Corner",
    "rating": "4.9",
    "food_type": "Japanese",
    "Price_for_two": 1198,
    "location": "Rajouri Garden",
    "Distance_from_Customer_house": "6.1",
    "alcohol": false,
    "Restaurant_open_time": 21,
    "Restaurant_close_time": 9,
    "offers": 20
  },
  {
    "image": "fourth",
    "name": "Savory Street",
    "rating": "2.2",
    "food_type": "Chinese",
    "Price_for_two": 958,
    "location": "Dwarka",
    "Distance_from_Customer_house": "8.7",
    "alcohol": true,
    "Restaurant_open_time": 6,
    "Restaurant_close_time": 18,
    "offers": 70
  },
  {
    "image": "ninth",
    "name": "Spice Symphony",
    "rating": "1.5",
    "food_type": "Korean",
    "Price_for_two": 566,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "3.1",
    "alcohol": false,
    "Restaurant_open_time": 0,
    "Restaurant_close_time": 12,
    "offers": 90
  },
  {
    "image": "ninth",
    "name": "Mediterranean Magic",
    "rating": "1.5",
    "food_type": "Mediterranean",
    "Price_for_two": 2234,
    "location": "Khan Market",
    "Distance_from_Customer_house": "8.2",
    "alcohol": false,
    "Restaurant_open_time": 3,
    "Restaurant_close_time": 15,
    "offers": 50
  },
  {
    "image": "second",
    "name": "Spice Symphony",
    "rating": "4.8",
    "food_type": "Korean",
    "Price_for_two": 901,
    "location": "Karol Bagh",
    "Distance_from_Customer_house": "8.6",
    "alcohol": false,
    "Restaurant_open_time": 5,
    "Restaurant_close_time": 17,
    "offers": 40
  },
  {
    "image": "fifth",
    "name": "Bistro Bliss",
    "rating": "2.8",
    "food_type": "Korean",
    "Price_for_two": 104,
    "location": "Rajouri Garden",
    "Distance_from_Customer_house": "9.8",
    "alcohol": false,
    "Restaurant_open_time": 2,
    "Restaurant_close_time": 14,
    "offers": 10
  },
  {
    "image": "fifth",
    "name": "Ocean Delights",
    "rating": "1.1",
    "food_type": "Thai",
    "Price_for_two": 1117,
    "location": "Saket",
    "Distance_from_Customer_house": "5.7",
    "alcohol": true,
    "Restaurant_open_time": 5,
    "Restaurant_close_time": 17,
    "offers": 40
  },
  {
    "image": "fifth",
    "name": "Spice Symphony",
    "rating": "2.4",
    "food_type": "Chinese",
    "Price_for_two": 1422,
    "location": "Vasant Kunj",
    "Distance_from_Customer_house": "4.7",
    "alcohol": false,
    "Restaurant_open_time": 4,
    "Restaurant_close_time": 16,
    "offers": 20
  },
  {
    "image": "third",
    "name": "Spice Symphony",
    "rating": "1.9",
    "food_type": "Indian",
    "Price_for_two": 1012,
    "location": "Saket",
    "Distance_from_Customer_house": "4.8",
    "alcohol": true,
    "Restaurant_open_time": 8,
    "Restaurant_close_time": 20,
    "offers": 60
  },
  {
    "image": "first",
    "name": "The Gourmet Haven",
    "rating": "4.1",
    "food_type": "Indian",
    "Price_for_two": 1494,
    "location": "Connaught Place",
    "Distance_from_Customer_house": "1.9",
    "alcohol": false,
    "Restaurant_open_time": 14,
    "Restaurant_close_time": 2,
    "offers": 20
  },
  {
    "image": "eighth",
    "name": "Golden Wok",
    "rating": "1.1",
    "food_type": "Mediterranean",
    "Price_for_two": 1543,
    "location": "Rajouri Garden",
    "Distance_from_Customer_house": "6.6",
    "alcohol": true,
    "Restaurant_open_time": 20,
    "Restaurant_close_time": 8,
    "offers": 20
  },
  {
    "image": "sixth",
    "name": "The Gourmet Haven",
    "rating": "4.4",
    "food_type": "American",
    "Price_for_two": 1697,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "6.8",
    "alcohol": false,
    "Restaurant_open_time": 11,
    "Restaurant_close_time": 23,
    "offers": 10
  },
  {
    "image": "ninth",
    "name": "Spice Symphony",
    "rating": "3.2",
    "food_type": "Korean",
    "Price_for_two": 1749,
    "location": "Saket",
    "Distance_from_Customer_house": "6.0",
    "alcohol": false,
    "Restaurant_open_time": 9,
    "Restaurant_close_time": 21,
    "offers": 40
  },
  {
    "image": "seventh",
    "name": "Breakfast Club",
    "rating": "4.2",
    "food_type": "American",
    "Price_for_two": 2195,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "3.3",
    "alcohol": false,
    "Restaurant_open_time": 0,
    "Restaurant_close_time": 12,
    "offers": 70
  },
  {
    "image": "eighth",
    "name": "The Green Plate",
    "rating": "3.1",
    "food_type": "Indian",
    "Price_for_two": 1404,
    "location": "Lajpat Nagar",
    "Distance_from_Customer_house": "10.8",
    "alcohol": false,
    "Restaurant_open_time": 6,
    "Restaurant_close_time": 18,
    "offers": 0
  },
  {
    "image": "sixth",
    "name": "Sunset Grill",
    "rating": "3.4",
    "food_type": "Japanese",
    "Price_for_two": 2100,
    "location": "Karol Bagh",
    "Distance_from_Customer_house": "9.6",
    "alcohol": true,
    "Restaurant_open_time": 20,
    "Restaurant_close_time": 8,
    "offers": 70
  },
  {
    "image": "sixth",
    "name": "The Green Plate",
    "rating": "2.8",
    "food_type": "Italian",
    "Price_for_two": 1143,
    "location": "Rajouri Garden",
    "Distance_from_Customer_house": "8.0",
    "alcohol": false,
    "Restaurant_open_time": 4,
    "Restaurant_close_time": 16,
    "offers": 40
  },
  {
    "image": "third",
    "name": "Spice Symphony",
    "rating": "4.5",
    "food_type": "Korean",
    "Price_for_two": 1930,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "5.4",
    "alcohol": false,
    "Restaurant_open_time": 0,
    "Restaurant_close_time": 12,
    "offers": 40
  },
  {
    "image": "third",
    "name": "The Chocolate Box",
    "rating": "4.5",
    "food_type": "Japanese",
    "Price_for_two": 2297,
    "location": "Rajouri Garden",
    "Distance_from_Customer_house": "4.1",
    "alcohol": false,
    "Restaurant_open_time": 20,
    "Restaurant_close_time": 8,
    "offers": 30
  },
  {
    "image": "sixth",
    "name": "The Chocolate Box",
    "rating": "4.6",
    "food_type": "Korean",
    "Price_for_two": 461,
    "location": "Dwarka",
    "Distance_from_Customer_house": "10.5",
    "alcohol": false,
    "Restaurant_open_time": 18,
    "Restaurant_close_time": 6,
    "offers": 50
  },
  {
    "image": "sixth",
    "name": "Pasta Paradise",
    "rating": "3.0",
    "food_type": "American",
    "Price_for_two": 529,
    "location": "Rajouri Garden",
    "Distance_from_Customer_house": "2.6",
    "alcohol": false,
    "Restaurant_open_time": 20,
    "Restaurant_close_time": 8,
    "offers": 50
  },
  {
    "image": "eighth",
    "name": "Spice Symphony",
    "rating": "4.6",
    "food_type": "Korean",
    "Price_for_two": 2427,
    "location": "Connaught Place",
    "Distance_from_Customer_house": "4.5",
    "alcohol": false,
    "Restaurant_open_time": 22,
    "Restaurant_close_time": 10,
    "offers": 20
  },
  {
    "image": "ninth",
    "name": "Spice Symphony",
    "rating": "3.7",
    "food_type": "Japanese",
    "Price_for_two": 786,
    "location": "Khan Market",
    "Distance_from_Customer_house": "1.7",
    "alcohol": false,
    "Restaurant_open_time": 0,
    "Restaurant_close_time": 12,
    "offers": 0
  },
  {
    "image": "first",
    "name": "Street Eats",
    "rating": "4.0",
    "food_type": "Indian",
    "Price_for_two": 1436,
    "location": "Vasant Kunj",
    "Distance_from_Customer_house": "4.3",
    "alcohol": true,
    "Restaurant_open_time": 3,
    "Restaurant_close_time": 15,
    "offers": 10
  },
  {
    "image": "third",
    "name": "Farm to Table",
    "rating": "1.1",
    "food_type": "Mexican",
    "Price_for_two": 1198,
    "location": "Lajpat Nagar",
    "Distance_from_Customer_house": "6.7",
    "alcohol": false,
    "Restaurant_open_time": 7,
    "Restaurant_close_time": 19,
    "offers": 10
  },
  {
    "image": "eighth",
    "name": "Breakfast Club",
    "rating": "3.0",
    "food_type": "French",
    "Price_for_two": 2047,
    "location": "Dwarka",
    "Distance_from_Customer_house": "4.2",
    "alcohol": false,
    "Restaurant_open_time": 21,
    "Restaurant_close_time": 9,
    "offers": 90
  },
  {
    "image": "eighth",
    "name": "The Gourmet Haven",
    "rating": "4.3",
    "food_type": "American",
    "Price_for_two": 2336,
    "location": "Rajouri Garden",
    "Distance_from_Customer_house": "7.7",
    "alcohol": false,
    "Restaurant_open_time": 3,
    "Restaurant_close_time": 15,
    "offers": 90
  },
  {
    "image": "fourth",
    "name": "Sushi Spot",
    "rating": "3.4",
    "food_type": "Indian",
    "Price_for_two": 443,
    "location": "Dwarka",
    "Distance_from_Customer_house": "1.8",
    "alcohol": true,
    "Restaurant_open_time": 7,
    "Restaurant_close_time": 19,
    "offers": 50
  },
  {
    "image": "fourth",
    "name": "Savory Street",
    "rating": "3.3",
    "food_type": "Chinese",
    "Price_for_two": 1214,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "2.9",
    "alcohol": false,
    "Restaurant_open_time": 23,
    "Restaurant_close_time": 11,
    "offers": 10
  },
  {
    "image": "fifth",
    "name": "Farm to Table",
    "rating": "4.7",
    "food_type": "Italian",
    "Price_for_two": 363,
    "location": "Saket",
    "Distance_from_Customer_house": "4.0",
    "alcohol": true,
    "Restaurant_open_time": 16,
    "Restaurant_close_time": 4,
    "offers": 30
  },
  {
    "image": "third",
    "name": "The Gourmet Haven",
    "rating": "4.3",
    "food_type": "Mediterranean",
    "Price_for_two": 1653,
    "location": "Dwarka",
    "Distance_from_Customer_house": "10.5",
    "alcohol": false,
    "Restaurant_open_time": 8,
    "Restaurant_close_time": 20,
    "offers": 20
  },
  {
    "image": "sixth",
    "name": "Pasta Paradise",
    "rating": "2.6",
    "food_type": "Mediterranean",
    "Price_for_two": 1832,
    "location": "Saket",
    "Distance_from_Customer_house": "3.0",
    "alcohol": true,
    "Restaurant_open_time": 4,
    "Restaurant_close_time": 16,
    "offers": 50
  },
  {
    "image": "second",
    "name": "The Rustic Table",
    "rating": "4.6",
    "food_type": "American",
    "Price_for_two": 1819,
    "location": "Hauz Khas Village",
    "Distance_from_Customer_house": "2.1",
    "alcohol": false,
    "Restaurant_open_time": 3,
    "Restaurant_close_time": 15,
    "offers": 0
  },
  {
    "image": "fourth",
    "name": "Sushi Spot",
    "rating": "4.0",
    "food_type": "Japanese",
    "Price_for_two": 1131,
    "location": "Karol Bagh",
    "Distance_from_Customer_house": "8.6",
    "alcohol": true,
    "Restaurant_open_time": 20,
    "Restaurant_close_time": 8,
    "offers": 0
  },
  {
    "image": "first",
    "name": "The Gourmet Haven",
    "rating": "2.8",
    "food_type": "Mexican",
    "Price_for_two": 2106,
    "location": "Dwarka",
    "Distance_from_Customer_house": "2.8",
    "alcohol": true,
    "Restaurant_open_time": 0,
    "Restaurant_close_time": 12,
    "offers": 30
  },
  {
    "image": "fifth",
    "name": "Breakfast Club",
    "rating": "1.7",
    "food_type": "Mexican",
    "Price_for_two": 540,
    "location": "Rajouri Garden",
    "Distance_from_Customer_house": "9.2",
    "alcohol": true,
    "Restaurant_open_time": 19,
    "Restaurant_close_time": 7,
    "offers": 40
  },
  {
    "image": "ninth",
    "name": "Ocean Delights",
    "rating": "2.6",
    "food_type": "Thai",
    "Price_for_two": 813,
    "location": "Lajpat Nagar",
    "Distance_from_Customer_house": "6.2",
    "alcohol": true,
    "Restaurant_open_time": 18,
    "Restaurant_close_time": 6,
    "offers": 70
  },
  {
    "image": "ninth",
    "name": "Sunset Grill",
    "rating": "4.8",
    "food_type": "Indian",
    "Price_for_two": 2371,
    "location": "Hauz Khas Village",
    "Distance_from_Customer_house": "1.1",
    "alcohol": false,
    "Restaurant_open_time": 7,
    "Restaurant_close_time": 19,
    "offers": 70
  },
  {
    "image": "fourth",
    "name": "Farm to Table",
    "rating": "4.8",
    "food_type": "Mexican",
    "Price_for_two": 2412,
    "location": "Karol Bagh",
    "Distance_from_Customer_house": "4.5",
    "alcohol": false,
    "Restaurant_open_time": 0,
    "Restaurant_close_time": 12,
    "offers": 50
  },
  {
    "image": "fifth",
    "name": "The Green Plate",
    "rating": "1.3",
    "food_type": "Korean",
    "Price_for_two": 1742,
    "location": "Rajouri Garden",
    "Distance_from_Customer_house": "3.0",
    "alcohol": false,
    "Restaurant_open_time": 4,
    "Restaurant_close_time": 16,
    "offers": 40
  },
  {
    "image": "first",
    "name": "Savory Street",
    "rating": "4.5",
    "food_type": "American",
    "Price_for_two": 2128,
    "location": "Hauz Khas Village",
    "Distance_from_Customer_house": "5.8",
    "alcohol": true,
    "Restaurant_open_time": 21,
    "Restaurant_close_time": 9,
    "offers": 10
  },
  {
    "image": "second",
    "name": "Sushi Spot",
    "rating": "3.9",
    "food_type": "American",
    "Price_for_two": 2121,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "6.5",
    "alcohol": false,
    "Restaurant_open_time": 21,
    "Restaurant_close_time": 9,
    "offers": 40
  },
  {
    "image": "sixth",
    "name": "Farm to Table",
    "rating": "2.0",
    "food_type": "Japanese",
    "Price_for_two": 2098,
    "location": "Khan Market",
    "Distance_from_Customer_house": "8.5",
    "alcohol": false,
    "Restaurant_open_time": 9,
    "Restaurant_close_time": 21,
    "offers": 90
  },
  {
    "image": "ninth",
    "name": "The Green Plate",
    "rating": "2.9",
    "food_type": "Italian",
    "Price_for_two": 2117,
    "location": "Lajpat Nagar",
    "Distance_from_Customer_house": "10.0",
    "alcohol": true,
    "Restaurant_open_time": 23,
    "Restaurant_close_time": 11,
    "offers": 60
  },
  {
    "image": "fourth",
    "name": "Bistro Bliss",
    "rating": "1.3",
    "food_type": "Korean",
    "Price_for_two": 2311,
    "location": "Saket",
    "Distance_from_Customer_house": "2.3",
    "alcohol": false,
    "Restaurant_open_time": 16,
    "Restaurant_close_time": 4,
    "offers": 0
  },
  {
    "image": "tenth",
    "name": "The Green Plate",
    "rating": "2.4",
    "food_type": "Italian",
    "Price_for_two": 821,
    "location": "Dwarka",
    "Distance_from_Customer_house": "3.3",
    "alcohol": false,
    "Restaurant_open_time": 15,
    "Restaurant_close_time": 3,
    "offers": 10
  },
  {
    "image": "fourth",
    "name": "Street Eats",
    "rating": "1.3",
    "food_type": "French",
    "Price_for_two": 1290,
    "location": "Lajpat Nagar",
    "Distance_from_Customer_house": "3.0",
    "alcohol": false,
    "Restaurant_open_time": 18,
    "Restaurant_close_time": 6,
    "offers": 0
  },
  {
    "image": "second",
    "name": "Urban Bites",
    "rating": "2.4",
    "food_type": "Mediterranean",
    "Price_for_two": 1930,
    "location": "Rajouri Garden",
    "Distance_from_Customer_house": "4.1",
    "alcohol": false,
    "Restaurant_open_time": 13,
    "Restaurant_close_time": 1,
    "offers": 10
  },
  {
    "image": "fourth",
    "name": "Pasta Paradise",
    "rating": "3.0",
    "food_type": "Mediterranean",
    "Price_for_two": 2076,
    "location": "Lajpat Nagar",
    "Distance_from_Customer_house": "4.7",
    "alcohol": true,
    "Restaurant_open_time": 13,
    "Restaurant_close_time": 1,
    "offers": 40
  },
  {
    "image": "fourth",
    "name": "Breakfast Club",
    "rating": "4.3",
    "food_type": "Italian",
    "Price_for_two": 2104,
    "location": "Khan Market",
    "Distance_from_Customer_house": "7.4",
    "alcohol": false,
    "Restaurant_open_time": 9,
    "Restaurant_close_time": 21,
    "offers": 90
  },
  {
    "image": "seventh",
    "name": "Breakfast Club",
    "rating": "1.8",
    "food_type": "French",
    "Price_for_two": 120,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "7.0",
    "alcohol": false,
    "Restaurant_open_time": 2,
    "Restaurant_close_time": 14,
    "offers": 90
  },
  {
    "image": "tenth",
    "name": "The Green Plate",
    "rating": "3.7",
    "food_type": "Indian",
    "Price_for_two": 1403,
    "location": "Hauz Khas Village",
    "Distance_from_Customer_house": "10.9",
    "alcohol": false,
    "Restaurant_open_time": 1,
    "Restaurant_close_time": 13,
    "offers": 30
  },
  {
    "image": "sixth",
    "name": "Ocean Delights",
    "rating": "3.7",
    "food_type": "French",
    "Price_for_two": 792,
    "location": "Khan Market",
    "Distance_from_Customer_house": "8.3",
    "alcohol": false,
    "Restaurant_open_time": 14,
    "Restaurant_close_time": 2,
    "offers": 40
  },
  {
    "image": "second",
    "name": "The Cozy Corner",
    "rating": "4.3",
    "food_type": "Mexican",
    "Price_for_two": 517,
    "location": "Connaught Place",
    "Distance_from_Customer_house": "2.3",
    "alcohol": false,
    "Restaurant_open_time": 23,
    "Restaurant_close_time": 11,
    "offers": 90
  },
  {
    "image": "eighth",
    "name": "Fusion Fiesta",
    "rating": "4.4",
    "food_type": "Mexican",
    "Price_for_two": 270,
    "location": "Hauz Khas Village",
    "Distance_from_Customer_house": "8.5",
    "alcohol": false,
    "Restaurant_open_time": 22,
    "Restaurant_close_time": 10,
    "offers": 10
  },
  {
    "image": "sixth",
    "name": "The Chocolate Box",
    "rating": "2.4",
    "food_type": "Mexican",
    "Price_for_two": 575,
    "location": "Dwarka",
    "Distance_from_Customer_house": "9.8",
    "alcohol": true,
    "Restaurant_open_time": 12,
    "Restaurant_close_time": 0,
    "offers": 90
  },
  {
    "image": "fourth",
    "name": "Golden Wok",
    "rating": "3.2",
    "food_type": "Indian",
    "Price_for_two": 1468,
    "location": "Karol Bagh",
    "Distance_from_Customer_house": "8.0",
    "alcohol": false,
    "Restaurant_open_time": 6,
    "Restaurant_close_time": 18,
    "offers": 0
  },
  {
    "image": "sixth",
    "name": "Bistro Bliss",
    "rating": "4.3",
    "food_type": "French",
    "Price_for_two": 537,
    "location": "Vasant Kunj",
    "Distance_from_Customer_house": "6.9",
    "alcohol": true,
    "Restaurant_open_time": 5,
    "Restaurant_close_time": 17,
    "offers": 10
  },
  {
    "image": "second",
    "name": "Mediterranean Magic",
    "rating": "2.8",
    "food_type": "Mediterranean",
    "Price_for_two": 2021,
    "location": "Dwarka",
    "Distance_from_Customer_house": "2.6",
    "alcohol": true,
    "Restaurant_open_time": 0,
    "Restaurant_close_time": 12,
    "offers": 60
  },
  {
    "image": "sixth",
    "name": "The Chocolate Box",
    "rating": "4.4",
    "food_type": "Mexican",
    "Price_for_two": 2397,
    "location": "Karol Bagh",
    "Distance_from_Customer_house": "3.0",
    "alcohol": false,
    "Restaurant_open_time": 9,
    "Restaurant_close_time": 21,
    "offers": 10
  },
  {
    "image": "eighth",
    "name": "The Chocolate Box",
    "rating": "2.5",
    "food_type": "Chinese",
    "Price_for_two": 1033,
    "location": "Khan Market",
    "Distance_from_Customer_house": "5.1",
    "alcohol": false,
    "Restaurant_open_time": 17,
    "Restaurant_close_time": 5,
    "offers": 30
  },
  {
    "image": "first",
    "name": "The Green Plate",
    "rating": "4.1",
    "food_type": "Italian",
    "Price_for_two": 378,
    "location": "Hauz Khas Village",
    "Distance_from_Customer_house": "5.0",
    "alcohol": false,
    "Restaurant_open_time": 16,
    "Restaurant_close_time": 4,
    "offers": 30
  },
  {
    "image": "fifth",
    "name": "Savory Street",
    "rating": "2.4",
    "food_type": "French",
    "Price_for_two": 481,
    "location": "Hauz Khas Village",
    "Distance_from_Customer_house": "4.2",
    "alcohol": false,
    "Restaurant_open_time": 12,
    "Restaurant_close_time": 0,
    "offers": 0
  },
  {
    "image": "sixth",
    "name": "Fusion Fiesta",
    "rating": "4.9",
    "food_type": "American",
    "Price_for_two": 331,
    "location": "Connaught Place",
    "Distance_from_Customer_house": "7.9",
    "alcohol": false,
    "Restaurant_open_time": 7,
    "Restaurant_close_time": 19,
    "offers": 40
  },
  {
    "image": "eighth",
    "name": "Burger Bliss",
    "rating": "2.9",
    "food_type": "Thai",
    "Price_for_two": 1465,
    "location": "Connaught Place",
    "Distance_from_Customer_house": "8.7",
    "alcohol": true,
    "Restaurant_open_time": 23,
    "Restaurant_close_time": 11,
    "offers": 70
  },
  {
    "image": "third",
    "name": "Fusion Fiesta",
    "rating": "4.6",
    "food_type": "Chinese",
    "Price_for_two": 1790,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "3.1",
    "alcohol": true,
    "Restaurant_open_time": 3,
    "Restaurant_close_time": 15,
    "offers": 0
  },
  {
    "image": "fourth",
    "name": "The Rustic Table",
    "rating": "2.6",
    "food_type": "Indian",
    "Price_for_two": 2398,
    "location": "Karol Bagh",
    "Distance_from_Customer_house": "3.8",
    "alcohol": false,
    "Restaurant_open_time": 10,
    "Restaurant_close_time": 22,
    "offers": 50
  },
  {
    "image": "second",
    "name": "Farm to Table",
    "rating": "1.2",
    "food_type": "American",
    "Price_for_two": 1085,
    "location": "Dwarka",
    "Distance_from_Customer_house": "8.4",
    "alcohol": false,
    "Restaurant_open_time": 0,
    "Restaurant_close_time": 12,
    "offers": 50
  },
  {
    "image": "eighth",
    "name": "Fusion Fiesta",
    "rating": "4.7",
    "food_type": "Chinese",
    "Price_for_two": 1853,
    "location": "Chandni Chowk",
    "Distance_from_Customer_house": "1.7",
    "alcohol": true,
    "Restaurant_open_time": 15,
    "Restaurant_close_time": 3,
    "offers": 70
  },
  {
    "image": "fifth",
    "name": "Golden Wok",
    "rating": "1.8",
    "food_type": "Korean",
    "Price_for_two": 914,
    "location": "Lajpat Nagar",
    "Distance_from_Customer_house": "8.6",
    "alcohol": false,
    "Restaurant_open_time": 1,
    "Restaurant_close_time": 13,
    "offers": 20
  },
  {
    "image": "second",
    "name": "Spice Symphony",
    "rating": "1.8",
    "food_type": "Japanese",
    "Price_for_two": 1442,
    "location": "Hauz Khas Village",
    "Distance_from_Customer_house": "8.5",
    "alcohol": false,
    "Restaurant_open_time": 12,
    "Restaurant_close_time": 0,
    "offers": 0
  },
  {
    "image": "first",
    "name": "Breakfast Club",
    "rating": "1.9",
    "food_type": "Indian",
    "Price_for_two": 1567,
    "location": "Hauz Khas Village",
    "Distance_from_Customer_house": "4.8",
    "alcohol": false,
    "Restaurant_open_time": 17,
    "Restaurant_close_time": 5,
    "offers": 50
  }
]

  function getrestaurants(res){
    res.forEach(res => {
        const root = document.getElementById('root')

        const card=document.createElement('div');
        card.classList.add('card')
        

        const ele = document.createElement('div');
        ele.classList.add('imgs');
        const image = document.createElement('img')
        image.classList.add('img');
        image.src=`Images/${res.image}.jpeg`
        ele.append(image);
        if(res.offers>0){
          const h6 = document.createElement('h2');
          h6.innerText = `${res.offers}% OFF`
          h6.classList.add('hey');
          ele.append(h6);
        }
      
        const card_content=document.createElement('div')
        card_content.classList.add('card-content')

        const card_header = document.createElement('div')
        card_header.classList.add('card-header')

        const h3 = document.createElement('h3')
        h3.textContent=res.name

        const rating= document.createElement('span')
        rating.textContent=res.rating + `\u2605`
        rating.classList.add('rating')
        rating.style.backgroundColor="green"

        card_header.appendChild(h3)
        card_header.appendChild(rating)

        const card_footer = document.createElement('div')
        card_footer.classList.add('card-footer')

        const food_type = document.createElement('span')
        food_type.textContent=res.food_type

        const price = document.createElement('span')
        price.textContent=`₹${res.Price_for_two}`

        card_footer.appendChild(food_type)
        card_footer.appendChild(price)

        const card_location = document.createElement('div')
        card_location.classList.add('card-location')

        const place = document.createElement('span')
        place.textContent=res.location

        const dis = document.createElement('span')
        dis.textContent=res.Distance_from_Customer_house + "km"

        card_location.appendChild(place)
        card_location.appendChild(dis)

        card_content.appendChild(card_header)
        card_content.appendChild(card_footer)
        card_content.appendChild(card_location)

        card.appendChild(ele)
        card.appendChild(card_content)

        root.appendChild(card)

    });
  }

  getrestaurants(res);

  document.getElementById('alcohol').addEventListener('click',()=>{
    document.getElementById('root').innerHTML=""
    const result=res.filter(obj=>obj.alcohol)
    getrestaurants(result);
  })

  document.getElementById('rating').addEventListener('click',()=>{
    document.getElementById('root').innerHTML=""
    const result=res.filter(obj=>obj.rating >=4.5)
    getrestaurants(result);
  })

  document.getElementById('filters').addEventListener('click',()=>{
    document.getElementById('filterPopup').classList.remove('hidden')
  })

  document.getElementById('apply').addEventListener('click',()=>{
    const element = document.querySelector('input[name="filterOption"]:checked')
    const ans = element.value;

    if(ans==="rating"){
      res.sort((a,b)=>b.rating-a.rating)
    }
    else if(ans==="highLow"){
      res.sort((a,b)=>b.Price_for_two-a.Price_for_two)
    }
    else if(ans==="costLowHigh"){
      res.sort((a,b)=>a.Price_for_two-b.Price_for_two)
    }
    else if(ans==="distance"){
      res.sort((a,b)=>a.Distance_from_Customer_house-b.Distance_from_Customer_house)
    }

    document.getElementById('root').innerHTML=""
    getrestaurants(res)
  })

  document.getElementById('close').addEventListener('click',()=>{
    document.getElementById('filterPopup').classList.add('hidden')
  })

  document.getElementById('open').addEventListener('click',()=>{
    document.getElementById('root').innerHTML=""
    const hours = new Date().getHours();
    const result = res.filter(obj=>(obj.Restaurant_open_time<=hours && hours<=obj.Restaurant_close_time))
    getrestaurants(result)
  })

  document.getElementById('offers').addEventListener('click',()=>{
    document.getElementById('root').innerHTML=""
    const result = res.filter(obj=>obj.offers>0);
    getrestaurants(result)
  })


